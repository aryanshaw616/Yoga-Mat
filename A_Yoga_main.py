...
def get_pushup_goal(age, weight, gender):
    """
    Returns the push-up goal based on age, weight, and gender.
    """
    age = int(age)
    weight = int(weight)
    if gender == "female":
        if 18 <= age <= 29:
            if weight < 70:
                return "44 or more", "Excellent"
            elif 70 <= weight <= 90:
                return "40 or more", "Excellent"
            else:
                return "35 or more", "Excellent"
        elif 30 <= age <= 39:
            if weight < 70:
                return "40 or more", "Excellent"
            elif 70 <= weight <= 90:
                ...
                return "30 or more", "Excellent"
    elif gender == "male":
        if 18 <= age <= 29:
            if weight < 60:
                return "24 or more", "Excellent"
            elif 60 <= weight <= 75:
                return "20 or more", "Excellent"
            else:
                return "15 or more", "Excellent"
        elif 30 <= age <= 39:
            if weight < 60:
                return "20 or more", "Excellent"
            elif 60 <= weight <= 75:
                return "18 or more", "Excellent"
            else:
                return "10 or more", "Excellent"
    return "10", "Average"
def pushups():
    ...
    with md_pose.Pose(
        ...
        while cap.isOpened():
            ...
            if len(imlist) != 0:
                ...
                if 'left_elbow_angle_smoothed' not in locals():
                    left_elbow_angle_smoothed = None
                if 'right_elbow_angle_smoothed' not in locals():
                ...
                hip_buffer = 10
                hips_low = (imlist[12][2] and imlist[11][2] >= (imlist[14][2] + hip_buffer) and imlist[13][2])
                ...
                elif hips_low and elbows_bent and not chest_down:
                    cv2.putText(image, "Lower Chest More!", (50, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                else:
                    position = None
                if (imlist[12][2] and imlist[11][2] <= imlist[14][2] and imlist[13][2] and position == "down"):
                    position = "up"
                    if (imlist[12][2] < imlist[14][2] - 10 and imlist[11][2] < imlist[13][2] - 10):  # adjust 10 as needed
                        count += 1
                        print(count)
